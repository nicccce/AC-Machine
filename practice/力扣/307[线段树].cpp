// https://leetcode.cn/problems/range-sum-query-mutable/description/?envType=problem-list-v2&envId=segment-tree
#include <bits/stdc++.h>
using namespace std;
...
inline long long part_sum(int index, int x, int y){
    ...
    void update(int index, int val) {
        update_one(1, index, val - w[index]);
        w[index] = val;
    }
    
    int sumRange(int left, int right) {
        return part_sum(1, left, right);
    }
};
