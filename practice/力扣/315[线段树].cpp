// https://leetcode.cn/problems/count-of-smaller-numbers-after-self/description/?envType=problem-list-v2&envId=segment-tree
#include <bits/stdc++.h>
using namespace std;
...
inline long long part_sum(int index, int x, int y){
    ...
    vector<int> countSmaller(vector<int>& nums) {
        ll py=2e4+5;
        w.resize(2*py);
        seg_tree.resize(2*py * 4);
        build_tree(1, 0, 2*py - 1);
        int n=nums.size();
        vector<int> res(n,0);
        for(int i=n-1;i>=0;i--){
            res[i]=part_sum(1,0,nums[i]+py-1);
            update_one(1,nums[i]+py,1);
        }
        return res;
    }
};
