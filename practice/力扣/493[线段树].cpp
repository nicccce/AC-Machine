// https://leetcode.cn/problems/reverse-pairs/submissions/692876982/?envType=problem-list-v2&envId=segment-tree
#include <bits/stdc++.h>
using namespace std;
...
inline long long part_sum(int index, int x, int y){
    ...
    int reversePairs(vector<int>& nums) {
        ...
        seg_tree.resize(allnums.size() * 4);
        build_tree(1, 0, allnums.size() - 1);
        ll res=0;
        for(auto i:nums){
            res+=part_sum(1,mp[2LL*i+1],allnums.size()-1);
            update_one(1,mp[i],1);
        }
        return res;
    }
