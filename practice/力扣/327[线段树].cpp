// https://leetcode.cn/problems/count-of-range-sum/?envType=problem-list-v2&envId=segment-tree
#include <bits/stdc++.h>
using namespace std;
...
inline long long part_sum(int index, int x, int y){
    ...
    int countRangeSum(vector<int>& nums, int lower, int upper) {
        ...
        for(int i=0;i<=n;i++){
            allnums.push_back(prefix[i]);
            allnums.push_back(prefix[i]-lower);
            allnums.push_back(prefix[i]-upper);
        }
        ...
        w.resize(allnums.size());
        seg_tree.resize(w.size() * 4);
        build_tree(1, 0, w.size() - 1);
        int ans=0;
        for(int i=0;i<=n;i++){
            ans+=part_sum(1,mp[prefix[i]-upper],mp[prefix[i]-lower]);
            update_one(1,mp[prefix[i]],1);
        }
        return ans;
    }
};
